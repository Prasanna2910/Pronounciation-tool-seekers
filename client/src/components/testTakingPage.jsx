...
Many successful people attribute their achievements to the habit of reading. By reading regularly, we can learn from the experiences of others and avoid making the same mistakes. Reading is truly a gift that keeps on giving throughout our lives.`,
    ...
    const handleSubmit = () => {
        ...
        // Calculate detailed result stats
        const wordCount = testData.passage.trim().split(/\s+/).length;
        const wpm = recordingTime > 0 ? Math.round((wordCount / recordingTime) * 60) : 0;
        // Mock pronunciation score for now (between 70-95%)
        const pronunciationScore = Math.floor(Math.random() * (95 - 70 + 1) + 70);
        
        const quizScore = Math.round((correctCount / testData.questions.length) * 100);
        const totalScore = Math.round((quizScore + pronunciationScore) / 2);
        ...
        const resultData = {
            ...
            feedback: "Good effort! You have a solid foundation but there's room for improvement. Keep practicing to improve reading speed and pronunciation clarity.",
        };
        
        // Submit test results (keep existing callback if needed)
        if (onSubmitTest) {
            onSubmitTest({
                level,
                day,
